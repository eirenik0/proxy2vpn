...
def test_system_diagnose_specific_container(monkeypatch):
    ...
    container = SimpleNamespace(name="vpn1", status="running", labels={"vpn.port": "8080"})
    ...
    async def mock_analyze_full_async(self, logs, container_name, port=None, include_control_server=True):
        return []
    
    monkeypatch.setattr(
        diagnostics.DiagnosticAnalyzer, 
        "analyze_full_async", 
        mock_analyze_full_async
    )
